...
describe('config loader', () => {
  it('applies precedence: defaults < global < project < overrides', async () => {
    const dir = tempDir();
    ...
    const noOverride = await loadConfig({ projectDir, globalDir });
...
describe('artifact store', () => {
  ...
  it('writes and reads structured artifacts', () => {
    ...
    const read = store.read<{ version: number }>('requirements');
